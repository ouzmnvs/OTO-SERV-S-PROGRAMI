...
# Font kayıt işlemi
try:
    # Arial Unicode MS fontunu kullan
    ...
    if os.path.exists(font_path) and os.path.getsize(font_path) > 0:
        ...
        # Arial Unicode MS bulunamazsa varsayılan fontları kullan
        print("Warning: arial-unicode-ms.ttf not found, using default fonts")
...
def mevcut_pdf_duzenle(kaynak_pdf, hedef_pdf, eklemeler, font_size=8):
    ...
    # Arial Unicode MS font dosyasının yolu
    font_path = resource_path('arial-unicode-ms.ttf')
    if not os.path.exists(font_path):
        print("Arial Unicode MS font dosyası bulunamadı, default font kullanılacak.")
        font_path = None
        ...
        if 'text' in eklemeler:
            for x, y, text in eklemeler['text']:
                ...
                page.insert_text(
                    ...
                    text,
                    fontsize=font_size,
                    fontname="ArialUnicode" if font_path else "helv",
                    fontfile=font_path,
                    color=(0, 0, 0)
                )
